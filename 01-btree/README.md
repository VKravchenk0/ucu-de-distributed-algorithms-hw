...
`B+Tree`: значення лежать тільки в листках. Внутрішні вузли зберігають лише сепаратор-ключі й дочірні page id.
...
| `src/page/node.rs` | B+Tree нода: серіалізація/десеріалізація, `split2`/`split3`, перевірка ліміту розміру запису (`max_kv_size`/`check_limits`). |
| `src/page/header.rs` | Header page файлу (magic, версія, page_size, root_id, лічильники алокатора). |
| `src/page/mod.rs` | Ре-експорт `header` і `node`. |
| `src/storage/tree.rs` | Сам алгоритм дерева: бінарний пошук у вузлі, рекурсивний `put` (path copying + split), `get`. |
| `src/storage/store.rs` | `Store`/`WriteTxn`/`ReadGuard` - COW commit-протокол, атомарна публікація root, лічильник активних читачів, `Mutex` для серіалізації паралельних записів. |
